...
createWikiModal() {
  ...
  closeBtn.shadow.setDepth(topDepth + 2);
  closeBtn.bg.setDepth(topDepth + 3);
  closeBtn.text.setDepth(topDepth + 4);
...
refreshWikiList() {
  ...
  let y = 0;
  const viewportW = Math.min(1060, this.scale.width - 40) - 48;
  ...
  if (this._wikiDetailUnit) {
    ...
    y = y + detailHeight + 8;
    this.wikiMaxScroll = Math.max(0, y - 660);
    this.wikiListContainer.y = this.wikiListBaseY;
    return;
  }
  
  // ---- UNIT LIST VIEW ----
  const allUnits = Object.values(UNIT_BY_ID);
  const itemSpacing = 8;
  
  allUnits.forEach((unit) => {
    const visual = getUnitVisual(unit.id, unit.classType);
    const skill = SKILL_LIBRARY?.[unit.skillId];
    const skillName = skill?.name ?? unit.skillId;
    
    const cardH = 72;
    const cardBg = this.add.rectangle(0, y, viewportW - 14, cardH, 0x0f1e30, 0.92).setOrigin(0, 0);
    cardBg.setStrokeStyle(1, 0x7ab8f5, 0.45);
    cardBg.setInteractive({ useHandCursor: true });
    this.wikiListContainer.add(cardBg);
    
    cardBg.on("pointerover", () => cardBg.setStrokeStyle(1, 0xffeab0, 0.9));
    cardBg.on("pointerout", () => cardBg.setStrokeStyle(1, 0x7ab8f5, 0.45));
    ...
    this.wikiListContainer.add(label);
